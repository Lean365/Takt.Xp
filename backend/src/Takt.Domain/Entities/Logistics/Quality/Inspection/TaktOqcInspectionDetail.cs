...
namespace Takt.Domain.Entities.Logistics.Quality.Inspection
{
    ...
    /// 出货检验明细（OQC）实体
    /// </summary>
    [SugarTable("Takt_logistics_quality_inspection_oqc_detail", "出货检验明细(OQC)表")]
    [SugarIndex("ix_inspection_no", nameof(InspectionNo), OrderByType.Asc, false)]
    public class TaktOqcInspectionDetail : TaktBaseEntity
    {
        /// <summary>
        /// 检验单号（外键）
        /// </summary>
        ...
        /// 导航属性：关联主表
        /// </summary>
        [Navigate(NavigateType.ManyToOne, nameof(InspectionNo), nameof(TaktOqcInspection.InspectionNo))]
        public TaktOqcInspection? OqcInspection { get; set; }
