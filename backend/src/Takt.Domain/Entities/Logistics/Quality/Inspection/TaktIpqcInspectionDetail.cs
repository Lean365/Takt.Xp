...
namespace Takt.Domain.Entities.Logistics.Quality.Inspection
{
    ...
    /// 过程检验明细（IPQC）实体
    /// </summary>
    [SugarTable("Takt_logistics_quality_inspection_ipqc_detail", "过程检验明细(IPQC)表")]
    [SugarIndex("ix_inspection_no", nameof(InspectionNo), OrderByType.Asc, false)]
    public class TaktIpqcInspectionDetail : TaktBaseEntity
    {
        /// <summary>
        /// 检验单号（外键）
        /// </summary>
        ...
        /// 导航属性：关联主表
        /// </summary>
        [Navigate(NavigateType.ManyToOne, nameof(InspectionNo), nameof(TaktIpqcInspection.InspectionNo))]
        public TaktIpqcInspection? IpqcInspection { get; set; }
